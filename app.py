...
# ================= 📊 PAINEL DE REGISTROS E EXPORTAÇÃO ================= #
else:
    ...
    try:
        ...
        if not df_hist.empty and 'Data' in df_hist.columns:
            ...
            # --- A MÁGICA DA DEDUPLICAÇÃO NATIVA DA DOCA ---
            # Removemos as antigas e ficamos com a última de cada Loja + Produto
            df_consolidado = df_dia.drop_duplicates(subset=['Loja', 'Fornecedor', 'Produto'], keep='last')
            linhas_ocultas = len(df_dia) - len(df_consolidado)
            
            msg_sucesso = f"✨ Visão Consolidada: Mostrando as contagens definitivas do dia {data_selecionada}."
            if linhas_ocultas > 0:
                msg_sucesso += f" ({linhas_ocultas} rascunhos antigos ignorados)."
            st.success(msg_sucesso)
            
            # Gera o CSV Perfeito
            csv = df_consolidado.to_csv(index=False).encode('utf-8')
            st.download_button(
                label="📥 Baixar CSV Consolidado (Pronto para Auditoria)",
                data=csv,
                file_name=f"Contagens_Doca_{str(data_selecionada).replace('/', '-')}.csv",
                mime="text/csv",
            )
            
            st.dataframe(df_consolidado, width='stretch', hide_index=True)
            
            with st.expander("Ver Histórico Bruto (com todas as correções antigas)"):
